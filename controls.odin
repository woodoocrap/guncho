...
// not a tool but an action actually
...
SwitchToolDown :: proc(self: ^Controls)
{
   ...
   self.selected_tool = Tool((int(self.selected_tool) + 1) % 6);
   if self.cooldowns[self.selected_tool] > 0 do SwitchToolDown(self);
}
...
{
   if self.sprinting == true do return;
   tool := (int(self.selected_tool) - 1);
   self.selected_tool = tool >= 0 ? Tool(tool) : Tool(5);
   if self.cooldowns[self.selected_tool] > 0 do SwitchToolUp(self);
}
